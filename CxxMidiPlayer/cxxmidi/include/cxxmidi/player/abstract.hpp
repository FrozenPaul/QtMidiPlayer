...
public:
    ...
    inline void repeatCallback(MyNoteCallaback* callback_);
    ...
    //add by PAul Halian
    inline void repeatcallBack(const std::function<void()> & callback);
#endif // __cplusplus > 199711L
...
protected:
    ...
    // add by Paul Halian
//    std::function<void(CxxMidi:Note, bool)> _noteCallBack;
#endif // __cplusplus > 199711L
...
void Abstract::setupWindowsTimers()
{
    ...
    if(once)
    {
        ...
        {
            UINT const one = 1;
            wTimerRes = std::min(std::max(tc.wPeriodMin, one /* [ms] */), tc.wPeriodMax);
...
//add by Paul Halian
void Abstract::repeatCallback(MyNoteCallaback* callback_)
{
    _note_is_pressed = callback_;
}
